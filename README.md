...
- 옵시디언 기본 볼트 위치(`~/Documents/Obsidian Vault`)가 존재하면 별도 설정 없이 자동으로 그 안의 `chzzk-chat-logs/{YYYY-MM-DD}` 폴더에 함께 저장됩니다.
- 타임스탬프 노트는 `chzzk-chat-logs/타임스탬프.md`에 기록되며, 수정 전 `.bak` 백업을 남깁니다.
